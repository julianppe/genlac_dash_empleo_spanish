...
dash.register_page(__name__,
                   path='/tasa-de-informalidad-laboral',  # represents the url text
                   name='Tasa de informalidad laboral',  # name of page, commonly used as name of link
                   title='Tasa de informalidad laboral'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/informal.csv")
...
df = df.query(f"indicador == 'Tasa de informalidad laboral'")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page4-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
